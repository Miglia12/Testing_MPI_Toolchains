...
The goal of this project is to validate the MPI library installed on the Uni.lu HPC platform under two different toolchains:
* toolchain/foss/2020b
* toolchain/foss/2023a
 
The ReFrame framework will be used to orchestrate sets of regression and sanity tests. We will leverage two sets of tests: the  [OSU benchmarking suite](http://mvapich.cse.ohio-state.edu/benchmarks/), and [Supermagic](https://github.com/hpc/supermagic).

## Folder structure

The OSU Benchmark tests include **sanity** and **performance** checks, and are categorized into 4 groups:
- Setup tests: `osu_init`, `osu_hello`
- Point-to-point tests: `osu_latency`, `osu_bw`
- Collective tests: `osu_alltoall`, `osu_bcast`, `osu_reduce` (and their non-blocking versions)
- One-sided tests: `osu_put_latency`, `osu_get_latency`, `osu_put_bw`, `osu_get_bw`

We created a separate ReFrame script for each of the test suites that you can find under `tests/osu_benchmarks`.

The Supermagic test suite contains only **sanity** checks and it can be found under the `tests/supermagic` folder.
...
```
## Output
After a successful run, the results will be stored in the `output` folder. The results of each test are stored in a separated subfolder and contain 3 files, generated by ReFrame:
- `rfm_job.err`: contains the error logs in case of failures
- `rfm_job.out`: contains the output of the benchmark test
- `rfm_job.sh`: contains the bash script generated and executed by ReFrame

In case of test failures, the inter-mediate results can be examined in the `stage` folder, generated by ReFrame. This folder will be automatically deleted if the run was successful.
